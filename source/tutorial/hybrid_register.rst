...
SCAKS supports more Pythonic way to register your own hybrid method to
our Hybrid Newton iteration flow.
...
1. ``model``: current micro-kinetic model instance You can obtain
   different components like parser, solver … to give you essential data
   for new coverages guess.

2. ``N``: hybrid method attemp times Sometimes the coverages guess is
...
Then when the model starts to run, the registered hybrid method would be
invoked everytime the Newton’s method fails to converge.
